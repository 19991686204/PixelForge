...
export default function SavedProjects() {
  ...
  const fetchSavedProjects = async () => {
    try {
      ...
      // 过滤出用户收藏的项目
      const backendSavedProjects = backendProjects.filter((project: any) =>
        project.saved.includes(userId)
      );
      
      // 从本地存储获取前端收藏的项目
      const frontendSaved = JSON.parse(localStorage.getItem('savedProjects') || '[]');
      
      // 合并后端和前端的收藏项目
      const allSavedProjects = [...backendSavedProjects];
      
      // 添加前端收藏的项目（如果不在后端列表中）
      frontendSaved.forEach((projectId: string) => {
        const mockProject = mockProjects.find(p => p._id === projectId);
        if (mockProject && !allSavedProjects.find(p => p._id === projectId)) {
          allSavedProjects.push(mockProject);
        }
      });
      ...
      // 错误时，从本地存储获取前端收藏的项目
      const frontendSaved = JSON.parse(localStorage.getItem('savedProjects') || '[]');
      const mockSavedProjects = mockProjects.filter(p => frontendSaved.includes(p._id));
      setProjects(mockSavedProjects);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUnsave = async (projectId: string) => {
    try {
      ...
      // 更新本地存储
      const frontendSaved = JSON.parse(localStorage.getItem('savedProjects') || '[]');
      const updatedSaved = frontendSaved.filter((id: string) => id !== projectId);
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="mb-8">
        ...
        <p style={{ color: '#6b7280' }}>{projects.length} projects you saved</p>
      </div>
